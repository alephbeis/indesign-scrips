...
// Fallback function for isValid
function isValidObject(obj) {
    if (typeof InDesignUtils !== "undefined" && InDesignUtils.Error && InDesignUtils.Error.isValid) {
        return InDesignUtils.Error.isValid(obj);
    }
...
(function () {
    var doc =
        typeof InDesignUtils !== "undefined" && InDesignUtils.Objects
    ...
    if (!doc) {
        showMessage("Object Position Checker", "Open a document first.");
        return;
    }
    ...
    app.doScript(
        function () {
            ...
            try {
                // Save and set preferences for performance
                try {
                    _origUnits = sp.measurementUnit;
                    sp.measurementUnit = MeasurementUnits.POINTS;
                } catch (_u0) {}
                try {
                    _origRedraw = sp.enableRedraw;
                ...
                function isOnMasterFrame(tf) {
                    try {
                        if (!isValidObject(tf)) return false;
                        var p = tf.parentPage;
                        if (!isValidObject(p)) return false; // pasteboard or detached
                ...
                function pageRelXY(item) {
                    var pg = item.parentPage;
                    if (!isValidObject(pg)) return null;
                    var a = anchorPB(item);
                    if (!a) return null;
                    var o = pageOrigin(pg);
                    return { x: a.x - o.x, y: a.y - o.y, page: pg };
                }
                function toPBFromPageRel(page, pr) {
                    var o = pageOrigin(page);
                    return { x: o.x + pr.x, y: o.y + pr.y };
                }
                function abs(v) {
                    return v < 0 ? -v : v;
                }
                ...
                function key(style, page) {
                    return (
                        (isValidObject(style) ? style.name : "<none>") + "@" + (isValidObject(page) ? page.id : "pb")
                    );
                }
                ...
                function buildLastSectionPageSet() {
                    ...
                    if (doc.sections.length === 0) {
                        // if no sections, treat entire doc as last section
                ...
                function expectedXY(style, page) {
                    if (!isValidObject(style) || !isValidObject(page)) return null;
                    ...
                    var tf1,
                        ...
                        res = null,
                        eps = 0.001;
                    var tempFrames = []; // Track frames for cleanup

                    try {
                        // Create frames with explicit cleanup tracking
                        tf1 = page.textFrames.add({
                            geometricBounds: [SEED1.y, SEED1.x, SEED1.y + TEMP_H, SEED1.x + TEMP_W]
                        });
                        tempFrames.push(tf1);
                        tf2 = page.textFrames.add({
                            geometricBounds: [SEED2.y, SEED2.x, SEED2.y + TEMP_H, SEED2.x + TEMP_W]
                        });
                        tempFrames.push(tf2);

                        // Apply style and check positions
                        tf1.appliedObjectStyle = style;
                        tf2.appliedObjectStyle = style;

                        after1 = pageRelXY(tf1);
                        after2 = pageRelXY(tf2);

                        if (after1 && after2 && abs(after1.x - after2.x) <= eps && abs(after1.y - after2.y) <= eps) {
                            res = { x: after1.x, y: after1.y };
                        }
                    } catch (_) {
                        // Silently handle errors - common in style testing
                    } finally {
                        // Clean up all temp frames
                        for (var i = 0; i < tempFrames.length; i++) {
                            try {
                                if (isValidObject(tempFrames[i])) tempFrames[i].remove();
                            } catch (_) {}
                        }
                    }
                    return res;
                ...
                function buildRowsNormal() {
                    ...
                    try {
                        for (var i = 0; i < frames.length; i++) {
                            // Update progress
                            if (progress && i % 10 === 0) {
                                var percent = Math.round((i / frames.length) * 100);
                                progress.update(percent, "Processing...");
                            }
                            var f = frames[i];
                            if (!isValidObject(f)) continue;
                            if (isOnMasterFrame(f)) continue; // exclude master pages
                            var pg = f.parentPage;
                            if (!isValidObject(pg)) continue;
                            if (!lastSectionPages[pg.id]) continue; // only last section

                            var st = f.appliedObjectStyle;
                            if (!isValidObject(st)) continue;
                ...
                var modeDrop = modeLeft.add("dropdownlist", undefined, ["Check mismatches", "Find styles without X/Y"]);
                ...
                function gotoItem(item) {
                    ...
                    try {
                        ...
                        if (row.ref.parentPage && row.ref.parentPage.isValid)
                            app.activeWindow.activePage = row.ref.parentPage;
                        app.select(NothingEnum.NOTHING);
                        app.select(row.ref);
                    } catch (e) {
                        showMessage("Object Position Checker", "Could not select that frame: " + e);
                    }
                }
                ...
                btnGoto.onClick = function () {
                    ...
                    if (!sel || (sel instanceof Array && !sel.length)) {
                        showMessage("Object Position Checker", "Select a row first.");
                        return;
                    }
                    ...
                    try {
                        list.selection = list.items[item.index];
                    } catch (_) {}
                ...
                btnGotoNext.onClick = function () {
                    ...
                    if (total === 0) {
                        showMessage("Object Position Checker", "No items to navigate.");
                        return;
                    }
                    var sel = list.selection;
                    var idx = -1;
                    ...
                    if (nextIdx >= total) {
                        showMessage("Object Position Checker", "Already at the last item.");
                        return;
                    }
                ...
                function fixRow(row) {
                    try {
                        ...
                        var wasLockedItem = false,
                            wasLockedLayer = false;
                        try {
                            if (row.ref.locked) {
                                wasLockedItem = true;
                                row.ref.locked = false;
                            }
                        } catch (_) {}
                        try {
                            if (row.ref.itemLayer && row.ref.itemLayer.locked) {
                                wasLockedLayer = true;
                                row.ref.itemLayer.locked = false;
                            }
                        } catch (_) {}
                        ...
                        try {
                            if (wasLockedItem) row.ref.locked = true;
                        } catch (_) {}
                        try {
                            if (wasLockedLayer) row.ref.itemLayer.locked = true;
                        } catch (_) {}
                        return true;
                    } catch (e) {
                        return false;
                ...
                btnFix.onClick = function () {
                    ...
                    var sel = list.selection;
                    if (!sel || (sel instanceof Array && !sel.length)) {
                        showMessage("Object Position Checker", "Select one or more rows first.");
                        return;
                    }
                    ...
                    if (fixed === 0) showMessage("Object Position Checker", "Nothing fixed.");
                };
                ...
                btnFixNext.onClick = function () {
                    ...
                    if (fixRow(row)) {
                        ...
                        showMessage("Object Position Checker", "Could not fix that item.");
                    }
                };
                ...
                btnFixAll.onClick = function () {
                    ...
                    var currentMode = modeDrop.selection && modeDrop.selection.index === 1 ? "reverse" : "normal";
                    updateButtonStates(currentMode);
                    if (fixed === 0) showMessage("Object Position Checker", "No items fixed.");
                };
                ...
                try {
                    if (_origUnits != null) sp.measurementUnit = _origUnits;
                } catch (_u1) {}
                try {
                    if (_origRedraw != null) sp.enableRedraw = _origRedraw;
                } catch (_r1) {}
            }
        },
