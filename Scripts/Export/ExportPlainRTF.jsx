        ...
        app.doScript(
            function () {
                ...
                try {
                    ...
                    while (tmpDoc.pages.length < srcDoc.pages.length) {
                        tmpDoc.pages.add();
                    }
                    while (tmpDoc.pages.length > srcDoc.pages.length) {
                        tmpDoc.pages.lastItem().remove();
                    }
                    ...
                    for (var pi = 0; pi < tmpDoc.pages.length; pi++) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                tf = dstPage.textFrames.add({
                                    geometricBounds: [fb[0] + 12, fb[1] + 12, fb[2] - 12, fb[3] - 12]
                                });
                            } catch (e2) {}
                        }
                    ...
                    for (var ti = 0; ti < dstFrames.length - 1; ti++) {
                        try {
                            dstFrames[ti].nextTextFrame = dstFrames[ti + 1];
                        } catch (_) {}
                    }
                    ...
                    for (var i = 0; i < srcDoc.pages.length; i++) {
                        ...
                        var progressText =
                            "Processing page " + srcPage.name + " (" + (i + 1) + "/" + srcDoc.pages.length + ")...";
                        progressController.update((i / srcDoc.pages.length) * 100, progressText);
                        ...
                        if (!pageText) {
                            pageText = "Page " + getDisplayPageLabel(srcPage);
                        }
                    ...
                    try {
                        ...
                        try {
                            mainStory.texts[0].appliedParagraphStyle = tmpDoc.paragraphStyles.item("Basic Paragraph");
                        } catch (_) {}
                        // Apply [None] character style if available (index 0)
                        try {
                            mainStory.texts[0].appliedCharacterStyle = tmpDoc.characterStyles[0];
                        } catch (_) {}
                        // Clear overrides if supported
                        try {
                            mainStory.texts[0].clearOverrides(OverrideType.ALL);
                        } catch (_) {}
                    } catch (_) {}

                    // Enforce center alignment for mixed-direction paragraphs
                    try {
                        tmpDoc.paragraphStyles.item("Basic Paragraph").justification = Justification.CENTER_ALIGN;
                    } catch (_) {}
                    try {
                        mainStory.paragraphs.everyItem().justification = Justification.CENTER_ALIGN;
                    } catch (_) {}

                    // Update progress to export
                    progressController.update(100, "Exporting RTF...");
                    ...
                    try {
                        if (tmpDoc && tmpDoc.isValid) tmpDoc.close(SaveOptions.NO);
                    } catch (_) {}
                    closeProgress();
