...
(function main() {
  ...
  function resolveOutputFolder(doc) {
    try {
      ...
      if (!docFolder) {
        try { if (doc && doc.fullName && doc.fullName.parent) docFolder = String(doc.fullName.parent.fsName || doc.fullName.parent); } catch (e2) {}
      }
      if (!docFolder) {
  ...
  function getBaseName(doc) {
    ...
    var n = 'Export';
    try { n = String((doc && doc.name) ? doc.name : 'Export'); } catch (_e) {}
    var i = n.lastIndexOf('.');
  ...
  function exportVariant(doc, opts) {
    ...
    try {
      ...
      // Restore prefs
      try { pdfPrefs.pageRange = orig.pageRange; } catch (_) {}
      try { pdfPrefs.exportReaderSpreads = orig.exportReaderSpreads; } catch (_) {}
      try { pdfPrefs.useSecurity = orig.useSecurity; } catch (_) {}
      try { pdfPrefs.viewPDF = orig.viewPDF; } catch (_) {}

      try { sp.measurementUnit = origSP.measurementUnit; } catch (_) {}
      try { sp.enableRedraw = origSP.enableRedraw; } catch (_) {}
    }
  }
  ...
  return runWithUndo('Export PDF (UXP)', function () {
    ...
    if (!app.documents || app.documents.length === 0) {
      try { alert('Open a document before running ExportPDF (UXP).'); } catch (_) {}
      return;
    }
    ...
    if (!preset) {
      try { alert('No PDF export presets found. Create a PDF preset and try again.'); } catch (_) {}
      return;
    }
